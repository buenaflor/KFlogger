 ...
 * LOG_AT_MOST_EVERY metadata key in [LogData]. Unlike the metadata for `every(N)`, we
 ...
 * via the `LOG_AT_MOST_EVERY` metadata key is required. This class implements `RateLimitStatus` as a mechanism for resetting the rate limiter state.
 *
 *
...
internal actual class KDurationRateLimiter : KRateLimitStatus() {
     ...
     */
    // Visible for testing.
    public actual fun checkLastTimestamp(timestampNanos: Long, period: KRateLimitPeriod): KRateLimitStatus {
        TODO()
    }
    ...
    actual companion object {
        /**
         * Creates a period for rate limiting for the specified duration. This is invoked by the [ ][LogContext.atMostEvery] method to create a metadata value.
         */
         ...
         */
        actual fun check(metadata: KMetadata, logSiteKey: KLogSiteKey?, timestampNanos: Long): KRateLimitStatus? {
            TODO()
        }
    }
}