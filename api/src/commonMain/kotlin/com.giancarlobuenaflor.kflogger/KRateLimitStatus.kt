 ...
 * <h2>Design Notes</h2>
 *
 * The purpose of this class is to allow rate limiters to behave in a way which is consistent when
 * multiple rate limiters are combined for a single log statement. If you are writing a rate limiter
 * for Flogger which you want to "play well" with other rate limiters, it is essential that you
 * understand how `RateLimitStatus` is designed to work.
 *
 * Firstly, `LogContext` tracks a single status for each log statement reached. This is modified by
 * code in the `postProcess()` method (which can be overridden by custom logger implementations).
 *
 * When a rate limiter is used, it returns a `RateLimitStatus`, which is combined with the existing
 * value held in the context:
...
/**
 * Rate limiters can extend this class directly if their "reset" operation is stateless, or they can
 * create and return new instances to capture any necessary state.
 */
protected constructor() {
   ...
   * limiting mode.
   *
   * Note: This method is never invoked concurrently with another `reset()` operation, but it can be
   * concurrent with calls to update rate limiter state. Thus it must be thread safe in general, but
   * can assume it's the only reset operation active for the limiter which returned it.
   */
  ...
  public companion object {
     ...
     *
     * Note: Truly stateless rate limiters should be *very* rare, since they cannot hold onto a
     * pending "allow" state. Even a simple "sampling rate limiter" should be stateful if once the
     * "allow" state is reached it continues to be returned until logging actually occurs.
     */
     ...
     *
     * In [LogContext] the `rateLimitStatus` field is set to the combined value of all rate limiter
     * statuses.
     *
     ...
     * `logSiteKey` (indicating that this log statement should be emitted) or `-1` if it should be
     * skipped.
     */
