 ...
 *
 *
 * Metadata keys can "single valued" or "repeating" based on [KMetadataKey.canRepeat], but
 * it is permitted for a `Metadata` implementation to retain multiple single valued keys, and
 ...
 *
 *
 * If the value of a single valued key is required, the [.findValue] method
 * should be used to look it up. For all other metadata processing, a [MetadataProcessor]
 * should be created to ensure that scope and log site metadata can be merged correctly.
 */
expect abstract class KMetadata {
     ...
     */
    abstract fun getKey(n: Int): KMetadataKey<*>?
     ...
     * @throws IndexOutOfBoundsException if either `n < 0` or {n >= getCount()}.
     */
    abstract fun getValue(n: Int): Any?
    ...
    abstract fun <T> findValue(key: KMetadataKey<T>?): T?
