 ...
 *
 *
 * This class allows configuration via a number of service types. A single instance of each
 * service type may be provided, either via the classpath using *service providers* (see [ ]) or by system property. For most users, configuring one of these should just
 * require including the appropriate dependency.
 *
 *
 * If set, the system property for each service type takes precedence over any implementations
 ...
 * public static no-arg `getInstance` method, the platform will call that instead.
 * **Note:** Support for `getInstance` is only provided to facilitate transition
 * from older service implementations that include a `getInstance` method and will
 * likely be removed in the future.
 *  * **A fully-qualified class name followed by "#" and the name of a static method:** In
 * this case, the platform will attempt to get an instance of that class by invoking either
 * the named no-arg static method or the public no-arg constructor. **Note:** This option
 ...
 * <td>[SimpleBackendFactory], a `java.util.logging` backend</td>
</tr> *
 * <tr>
 * <td>[ContextDataProvider]</td>
 * <td>`flogger.logging_context`</td>
 * <td>A no-op `ContextDataProvider`</td>
</tr> *
 * <tr>
 * <td>[Clock]</td>
 * <td>`flogger.clock`</td>
 * <td>[SystemClock], a millisecond-precision clock</td>
</tr> *
</table> *
 */
