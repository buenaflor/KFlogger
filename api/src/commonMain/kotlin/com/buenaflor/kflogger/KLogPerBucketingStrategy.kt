 ...
 * the [&lt;][KLoggingApi.per] method.
 *
 *
 * When implementing new strategies not provided by this class, it is important to ensure that
 * the `apply()` method returns values from a bounded set of instances wherever possible.
 *
 *
 ...
 * retained indefinitely by any use of [&lt;][KLoggingApi.per].
 *
 *
 ...
 */
expect abstract class KLogPerBucketingStrategy<T> protected constructor(name: String?) {
    /**
     ...
     * requirements and ideally have singleton semantics (e.g. an `Enum` or `Integer`
     * value).
     *
     *
     * *Warning*: If keys are not known to have natural singleton semantics (e.g. `String`) then returning the given key instance is generally a bad idea. Even if the set of key
     * values is small, the set of distinct allocated instances passed to [ ][KLoggingApi.per] can be unbounded, and that's what matters. As
    ...
    companion object {
         ...
         *
         *
         * *WARNING*: When using this strategy, keys passed to [ ][KLoggingApi.per] are used as-is by the log aggregation code, and
         * held indefinitely by internal static data structures. As such it is vital that key instances
         * used with this strategy have singleton semantics (i.e. if `k1.equals(k2)` then `k1
         ...
         */
        fun knownBounded(): KLogPerBucketingStrategy<Any>
         ...
         *
         *
         * Note that using this strategy will result in a reference to the `Class` object of the
         ...
         */
        fun byClass(): KLogPerBucketingStrategy<Any>
         ...
         * rare.
         */
        fun byClassName(): KLogPerBucketingStrategy<Any>
         ...
         * permits the use of non-singleton keys in [&lt;][KLoggingApi.per].
         *
         *
         ...
         */
        fun forKnownKeys(knownKeys: Iterable<*>): KLogPerBucketingStrategy<Any>
         ...
         *
         *
         * When using this method it is obviously important that the `hashCode()` method of the
         ...
         * keys when performing log aggregation. Each log site using this strategy will hold up to `maxBuckets` distinct versions of log site information to allow rate limiting and other stateful
         * operations to be applied separately per bucket. The overall allocation cost depends on the type
         * of rate limiting used alongside this method, but it scales linearly with `maxBuckets`.
         *
         *
         * It is recommended to keep the value of `maxBuckets` below 250, since this guarantees
         ...
         */
        fun byHashCode(maxBuckets: Int): KLogPerBucketingStrategy<Any>
    }
}
