 ...
 *
 * It is also essential that no implementation of `LoggerBackend` ever holds onto user supplied
 * objects (especially log statement arguments) after the `log()` or `handleError()` methods to
 * which they were passed have exited.
 *
 * This means that *ALL* formatting or serialization of log statement arguments or metadata values
 * *MUST* be completed inside the log method itself. If the backend needs to perform asynchronous
 * I/O operations it can do so by constructing a serialized form of the [ ] instance and enqueing
 * that for processing.
 *
 * Note also that this restriction is *NOT* purely about mutable arguments (which could change
 * before formatting occurs and produce incorrect output), but also stops log statements from
 * changing the lifetime of arbitrary user arguments, which can cause "use after close" bugs and
 * other garbage collector issues.
 */
public actual abstract class LoggerBackend {
  /**
   * Returns the logger name (which is usually a canonicalized class name) or `null` if not given.
   */
  public actual abstract val loggerName: String
   ...
   */
  public actual abstract fun isLoggable(lvl: Level?): Boolean
   ...
   */
  public actual abstract fun log(data: LogData?)
   ...
   */
  public actual abstract fun handleError(error: RuntimeException?, badData: LogData?)
}
