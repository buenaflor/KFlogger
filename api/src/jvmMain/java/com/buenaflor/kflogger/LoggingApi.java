...
public interface LoggingApi<API extends LoggingApi<API>> {
   ...
   * provider.
   *
   * <p>When {@link com.buenaflor.kflogger.context.ScopedLoggingContext ScopedLoggingContext} is
   * used to create a context, it can be bound to a {@link
   * com.buenaflor.kflogger.context.ScopeType ScopeType}. For example:
   *
   * <pre>{@code
   * ScopedLoggingContexts.newContext(REQUEST).run(() -> scopedMethod(x, y, z));
   * }</pre>
   *
   * where {@link com.buenaflor.kflogger.context.ScopeType#REQUEST REQUEST} defines the scope
   ...
   * com.buenaflor.kflogger.context.ScopeType#create ScopeType.create("<name>")} but it is
