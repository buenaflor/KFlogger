...
public interface LoggingApi<API extends LoggingApi<API>> {
   ...
   * provider.
   *
   * <p>When {@link com.giancarlobuenaflor.kflogger.context.ScopedLoggingContext ScopedLoggingContext} is
   * used to create a context, it can be bound to a {@link
   * com.giancarlobuenaflor.kflogger.context.ScopeType ScopeType}. For example:
   *
   * <pre>{@code
   * ScopedLoggingContexts.newContext(REQUEST).run(() -> scopedMethod(x, y, z));
   * }</pre>
   *
   * where {@link com.giancarlobuenaflor.kflogger.context.ScopeType#REQUEST REQUEST} defines the scope
   ...
   * com.giancarlobuenaflor.kflogger.context.ScopeType#create ScopeType.create("<name>")} but it is
