...
/**
 * A logger backend which captures all `LogData` instances logged to it. This class is
...
class KLogDataSubject private constructor(
    ...
    fun hasMessage(messageOrLiteral: Any?) {
        if (actual?.getTemplateContext() == null) {
            ...
            androidx.kruth.assertThat(listOf(actual?.getLiteralArgument())).containsExactly(messageOrLiteral)
        } else {
            androidx.kruth.assertThat(actual?.getTemplateContext()?.getMessage()).isEqualTo(messageOrLiteral)
        }
    }
